...
export const teamsReducer = (state = { loading: false, error: null, data: [] }, action) => {
  switch (action.type) {
    ...
    case EDIT_TEAM: {
      const data = state.data.map((r) => (r._id === action.payload._id ? { ...r, ...action.payload } : r));
...
export const acceptedTeamsReducer = (
  ...
  switch (action.type) {
    ...
    case EDIT_ACCEPTED_TEAM: {
      const {
        title, description, startDate, endDate, rest,
      } = action.payload;
      const data = state.data.map((d) => (d._id === action.payload._id
        ? {
          ...d,
          ...rest,
          repository: {
    ...
    case ADD_TEAM_MEMBER: {
      const data = state.data.map((d) => (d._id === action.payload.teamId
